...
Функция СведенияОВнешнейОбработке() Экспорт 
	...
	ПараметрыРегистрации.Информация = "Внешняя печатная форма МХ-3 для документа ""Акт отгрузки"". Задача № 2545 Профрешение";
...
Процедура Печать(МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "МХ3", Нстр("ru = 'Печать МХ-3'"), ТабличныйДокументПечатьМХ3);
...
Процедура ПечатьШапки(ТабДокумент, Макет, СсылкаНаДокумент) 
	...
	ЗапросШапка.Текст = 
	"ВЫБРАТЬ
	|	уатАктОтгрузки_уэ.Номер КАК НомерДокумента,
	|	уатАктОтгрузки_уэ.Дата КАК ДатаДокумента,
	|	уатАктОтгрузки_уэ.ПР_Контрагент КАК Контрагент,
	|	уатАктОтгрузки_уэ.Склад КАК Склад,
	...
	|	Склады.Представление КАК ПредставлениеПодразделения
	|ИЗ
	|	Документ.уатАктОтгрузки_уэ КАК уатАктОтгрузки_уэ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации КАК Организации
	|		ПО уатАктОтгрузки_уэ.Организация = Организации.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	|		ПО уатАктОтгрузки_уэ.ПР_Договор = ДоговорыКонтрагентов.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
	|		ПО уатАктОтгрузки_уэ.ПР_Контрагент = Контрагенты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК Склады
	|		ПО уатАктОтгрузки_уэ.Склад = Склады.Ссылка
	|ГДЕ
...
Процедура ПечатьСтрок(ТабДокумент, Макет, СсылкаНаДокумент)
	...
	|	уатАктОтгрузки_уэГрузы.ЕдиницаИзмерения КАК ЕдиницыИзмеренияНаименование,
	|	уатАктОтгрузки_уэГрузы.Количество КАК КоличествоВОсновнойУпаковке,
	|	уатАктОтгрузки_уэГрузы.Количество КАК КоличествоМест,
	...
	|		ПО уатАктОтгрузки_уэГрузы.ГрузовоеМесто = Номенклатура.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.уатВидыУпаковки_уэ КАК уатВидыУпаковки_уэ
	|		ПО уатАктОтгрузки_уэГрузы.ЕдиницаИзмерения = уатВидыУпаковки_уэ.Ссылка";
...
Процедура ПечатьПодвала(ТабДокумент, Макет, СсылкаНаДокумент)
	...
	ЗапросПодвал.Текст = 
	"ВЫБРАТЬ
	|	ДолжностиСдал.Представление КАК ДолжностьСдал,
	|	ДолжностиПринял.Представление КАК ДолжностьПринял,
	|	СотрудникПринял.Наименование КАК РасшифровкаПодписиПринял,
	|	СотрудникСдал.Наименование КАК РасшифровкаПодписиСдал,
	|	уатАктОтгрузки_уэ.ПР_ОсобыеОтметки КАК ПР_ОсобыеОтметки
	|ИЗ
	|	Документ.уатАктОтгрузки_уэ КАК уатАктОтгрузки_уэ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Сотрудники КАК СотрудникСдал
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Должности КАК ДолжностиСдал
	|			ПО СотрудникСдал.Должность = ДолжностиСдал.Ссылка
	|		ПО уатАктОтгрузки_уэ.Отпустил = СотрудникСдал.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Сотрудники КАК СотрудникПринял
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Должности КАК ДолжностиПринял
	|			ПО СотрудникПринял.Должность = ДолжностиПринял.Ссылка
	|		ПО уатАктОтгрузки_уэ.Получил = СотрудникПринял.Ссылка
	|ГДЕ
	|	уатАктОтгрузки_уэ.Ссылка = &ТекущийДокумент";
