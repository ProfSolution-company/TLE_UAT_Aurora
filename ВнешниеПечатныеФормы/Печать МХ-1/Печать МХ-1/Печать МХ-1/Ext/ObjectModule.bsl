...
Процедура ПечатьШапки(ТабДокумент, Макет, СсылкаНаДокумент) 
	...
	ОКПОПокупателя = Строка(Шапка.Контрагент.КодПоОКПО);
	...
	ОбластьМакета.Параметры.ОКПОПокупателя = ОКПОПокупателя;
...
Процедура ПечатьСтрок(ТабДокумент, Макет, СсылкаНаДокумент)
	...
	ЗапросСтрока.Текст = "ВЫБРАТЬ
	                     ...
	                     |	уатАктПриемки_уэГрузы.ГрузовоеМесто.НаименованиеПолное КАК ПолноеНаименование,
	                     |	уатАктПриемки_уэГрузы.Количество КАК Количество,
	                     |	уатАктПриемки_уэГрузы.ГрузовоеМесто.уатВидУпаковкиОсновной.Коэффициент КАК КоличествоМест
	                     |ИЗ
	...
	Для Каждого Текущий Из Строки Цикл 
		...
		ОбластьМакетаСтроки.Параметры.КоличествоВОсновнойУпаковке = Текущий.Количество;
		...
		ВсегоВес =   ВсегоВес +  Текущий.Количество; 
		ИтогоВес =   ИтогоВеС +  Текущий.Количество;
